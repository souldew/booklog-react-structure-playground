...
| エラー | `apis/` が `ApiError` を throw し、ルートの `error.tsx` が受ける。`app/books/error.tsx` と `app/dashboard/error.tsx` にあり、**`reset` ではなく `retry`** で再試行する。`reset` は再取得せずに描き直すだけなので、取得の失敗では同じ結果に戻る |
